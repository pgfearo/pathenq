...
<xsl:variable name="index" select="0" as="xs:integer"/>
<xsl:result-document href="file:///source{$index}" method="replace-content">
<xsl:sequence select="f:render()"/>
</xsl:result-document>
<ixsl:schedule-action wait="20">
<xsl:call-template name="resultDocumentCallback">
<xsl:with-param name="index" select="$index"/>
</xsl:call-template>
</ixsl:schedule-action>
</xsl:template>

<xsl:template name="resultDocumentCallback">
<xsl:param name="index"/>
<xsl:sequence select="js:onCurrentXmlRendered($index)"/>
</xsl:template>
...
<!-- deleting the following LRE gives the error: 'cannot swich to final
     result destination while updating a
     temporary tree -->
<span/>
<!--
...
<xsl:variable name="fileIndex" select="(($counter -1) div 20) + 1"/>

<!--
<xsl:message><xsl:value-of select="'counter:',$counter,'fileIndex:',$fileIndex, 'index',$index, 'total:', count($tokens)"/></xsl:message>
-->

<xsl:result-document href="file:///source{$fileIndex}" method="append-content">
<xsl:sequence select="f:iterateTokens($counter, $tokens, $index, $expected, $beganAt, $level)"/>
</xsl:result-document>
<ixsl:schedule-action wait="20">
<xsl:call-template name="resultDocumentCallback">
<!-- eg. counter = 21 thus index = (20 - 1) div 20 = 1 -->
<xsl:with-param name="index" select="$fileIndex"/>
</xsl:call-template>
</ixsl:schedule-action>
...
<span class="txt" id="{js:getStackIndex()}">
...
<!--
<span style="color:red">Level: <xsl:value-of select="$newLevel"/></span>
-->
...
<xsl:choose>
<xsl:when test="$index le count($tokens)">
<xsl:choose>

<xsl:when test="$counter mod 20 eq 0">
...
<xsl:otherwise>
<!-- make callback with -1 to say complete -->
<ixsl:schedule-action wait="20">
<xsl:call-template name="resultDocumentCallback">
...
<xsl:if test="$index eq 1">
<span class="es">&lt;</span>
<span class="en" id="{js:stackPush($elementName)}">
